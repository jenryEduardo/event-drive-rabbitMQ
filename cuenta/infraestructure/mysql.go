...
func (r *MySQLRepository) Save(p *domain.Cuenta) error {
	query := "INSERT INTO cuenta (titular, saldo,moneda,creado_en) VALUES (?, ?,?,?)"
	_, err := r.conn.DB.Exec(query, p.Titular,p.Saldo,p.Moneda,p.Creado_en)
	return err
}
